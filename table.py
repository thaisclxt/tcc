...
def generate_table(is_MG, function, lambda_row, tolerance_row, time_row, k_row, x_row, norm_row):
	...
	for i in range(rows):
		if is_MG:
			table.add_row(str(i), tolerance_row[i], time_row[i], k_row[i], x_row[i], norm_row[i])
		else:
			table.add_row(str(i), tolerance_row[i], lambda_row[i], time_row[i],k_row[i], x_row[i], norm_row[i])
